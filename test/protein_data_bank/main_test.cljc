(ns protein-data-bank.main-test
  "Contract + behavioral test for the protein_data_bank-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "protein_data_bank-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'protein-data-bank.main-test)]
