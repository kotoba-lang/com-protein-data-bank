(ns protein-data-bank.main
  "Kotodama WASM entrypoint for the Protein_data_bank clean-room actor (L4) — Clojure port.
  ...
  facts (`protein_data_bank.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "protein_data_bank")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"protein_data_bank.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "protein_data_bank-compat" :tier tier :entities entities} 200])
...
(defn start! [] :protein_data_bank-compat/ready)
